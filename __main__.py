...
register_auto_tags(
    {
        ...
        "service": pulumi.get_project(),
        "user:Stack": pulumi.get_stack(),
        "user:email": "corey@duckbillgroup.com",
    }
)
...
content_bucket = pulumi_aws.s3.Bucket(
    ...
    acl="private",
    lifecycle_rules=[
        pulumi_aws.s3.BucketLifecycleRuleArgs(
            enabled=True,
            id="intelligent-tiering",
            transitions=[
                pulumi_aws.s3.BucketLifecycleRuleTransitionArgs(
                    days=0,
...
origin_access_identity = pulumi_aws.cloudfront.OriginAccessIdentity(
    "cdn",
    comment=pulumi.Output.concat(f"OAI-{content_bucket.bucket_domain_name}"),
)
...
logs_bucket = pulumi_aws.s3.Bucket(
    "requestLogs", bucket=f"{target_domain}-logs", acl="private"
)
...
bucket_policy = pulumi_aws.s3.BucketPolicy(
    ...
    policy=pulumi.Output.all(
        cloudfront_iam_arn=origin_access_identity.iam_arn, bucket_arn=content_bucket.arn
    ).apply(
